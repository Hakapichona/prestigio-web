<script setup>
	...
	const props = defineProps({
		navItems: {
			type: Array,
			default: () => ["Inicio", "Nosotros", "Servicios", "Páginas", "Blog"],
		},
		brand: {
			type: String,
	...
	>
		<nav class="ps-w-full ps-max-w-md ps-px-6">
			...
			<div class="ps-mt-10 ps-text-center">
				<button
					class="ps-bg-red-600 ps-text-white ps-px-8 ps-py-3 ps-rounded ps-text-lg ps-hover:ps-bg-red-700 ps-transition-colors ps-duration-300 ps-transform ps-transition-transform ps-duration-300 hover:ps-scale-105"
				>
					Contáctanos
				</button>
			</div>
...
<script>
	// Script adicional para las partículas
	export default {
		methods: {
